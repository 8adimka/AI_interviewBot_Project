...
async def start_handler(update: Update, context):
    ...
    try:
        ...
        welcome_message = openai_client.prompt_model(
            Prompt(text=interview_prompt.text.format(max_questions=max_questions))
        )
        await update.message.reply_text(welcome_message)

        await send_next_question(
            update, context, user_name, openai_client, interview_repo
        )
...
async def handle_response(update: Update, context):
    ...
    try:
        ...
        await send_next_question(
            update, context, user_name, OpenAiClient(), interview_repo
        )
    except Exception as e:
        print(f"Error occurred: {e}")
        await update.message.reply_text(
...
async def send_next_question(
    update: Update, context, user_name, openai_client, interview_repo
):
    try:
        ...
        await update.message.reply_text(
            "Произошла ошибка при получении следующего вопроса. Попробуйте позже."
        )
